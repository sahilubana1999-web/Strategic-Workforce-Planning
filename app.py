...
prod_method = st.sidebar.radio("Productivity per employee (monthly)", ["Use recent observed (avg)", "Manual input"])
recent_months = st.sidebar.slider("If using observed: months to average", 3, 24, 6)
...
df['avg_employees'] = df['employees']  # leave room for more advanced logic

if prod_method == "Use recent observed (avg)":
    recent = df.tail(recent_months)
    avg_output_month = recent['output'].mean()
    avg_emp_month = recent['employees'].mean()
    if avg_emp_month <= 0:
        st.warning("Average employees is zero or invalid. Switching to manual input.")
        prod = st.sidebar.number_input("Manual productivity (output per employee per month)", 0.1, 100000.0, 20.0)
    else:
...
# Footer: literature and references
st.markdown("---")
st.markdown("#### Literature & practical sources used")
st.markdown("""
- Strategic Workforce Planning & why workforce forecasting matters — industry guidance (Deloitte).
- Workforce planning frameworks and HR-finance integration (HBR/SHRM discussions).
- Prophet for time series forecasting (installation & use).
- ARIMA & statsmodels reference for time-series forecasting in Python.
""")
